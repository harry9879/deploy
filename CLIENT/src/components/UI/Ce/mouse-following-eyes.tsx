...
const Eye: React.FC<EyeProps> = ({ mouseX, mouseY, selfRef, otherRef }) => {
  ...
  return (
    <div
      ref={selfRef}
      className="relative bg-white border-8 border-gray-900 rounded-full h-40 w-40 flex items-center justify-center shadow-2xl"
    >
      <div
        ref={pupilRef}
        className="absolute bg-gray-900 rounded-full h-16 w-16 transition-all duration-[5ms]"
      >
        <div className="w-6 h-6 bg-white rounded-full absolute bottom-2 right-2"></div>
      </div>
    </div>
  );
};
